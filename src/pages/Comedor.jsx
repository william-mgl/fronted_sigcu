...
import { FaClock, FaShoppingCart, FaTimesCircle } from "react-icons/fa";
...
export default function Comedor() {
  ...
  const cargarDatos = () => {
    fetch(`${API_URL}/api/comedores/${comedorId}`)
      .then(res => res.json())
      .then(data => setComedor(data))
  ...
  const cancelarReserva = async (reservaId) => {
    try {
      const res = await fetch(`${API_URL}/api/reservas/${reservaId}`, { method: "DELETE" });
      const data = await res.json();
      if (res.ok) {

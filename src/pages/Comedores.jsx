...
export default function Comedores() {
  ...
  return (
    <div className="d-flex flex-column align-items-center min-vh-100 text-white p-4"
         style={{ background: "linear-gradient(135deg, #0f2027, #203a43, #2c5364)" }}>
      <header className="w-100 d-flex justify-content-between align-items-center mb-4">
        <button className="btn btn-outline-light d-flex align-items-center gap-2 fw-semibold"
                onClick={() => navigate("/dashboard")}>
          <FaArrowLeft /> Volver
        </button>
        <h2 className="fw-bold">Comedores de la Facultad</h2>
        <div></div>
      </header>

      <main className="container d-flex flex-wrap justify-content-center gap-4">
        {comedores.length === 0 && <p className="text-white fs-5 text-center w-100 mt-5">No hay comedores disponibles en esta facultad</p>}
        {comedores.map(c => (
          <div key={c.id} className="col-md-3 p-4 rounded-4 cursor-pointer d-flex flex-column align-items-center text-center glass-card"
